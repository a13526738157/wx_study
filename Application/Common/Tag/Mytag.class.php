 ...
 * @Last Modified time: 2016-12-06 10:49:03
 */
...
class Mytag extends TagLib
{
	...
	*/
	...
	public function _FinishedOption($tag,$content){
		...
		$brand_id = session('userinfo.brand_id');
		$uid = session('userinfo.uid');
		$shows = D('Admin/Goodstyperelation')->getSaleset(array('uid'=>$uid,'brand_id'=>$brand_id,'is_show'=>1,'paramset_id'=>array('neq',0),'goodstype_id'=>$info['id']));
		...
		$str = '<?php ';
       	$str .= '$list = D("Admin/Paramsetvalue")->getSearchOption(array("goodstype_id"=>'.$info['id'].',"is_search"=>'.$is_search.',"search_type"=>array('.$search_type.'),"paramset_ids"=>array('.implode(',',$sales).'),"brand_id"=>'.$brand_id.',"is_show"=>'.$is_show.'));';
       	$str .= 'foreach ($list as $'.$id.'): ?>';
       	$str .= $content;
       	$str .='<?php endforeach ?>';
       	return $str;
