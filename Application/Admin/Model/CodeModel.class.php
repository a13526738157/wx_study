 ...
 * @Last Modified time: 2016-11-17 22:00:39
 */
...
class CodeModel extends Model
{
	...
	protected $col = 'name,code,val,is_system,trees,ord,pid,uid,modify_time';
    ...
    public function getData($row){
        ...
        if(isset($row['pid'])){
            $this->where(array('pid'=>$row['pid']));
        }
        if(isset($row['pid_neq'])){
            $this->where(array('pid'=>array('neq',$row['pid_neq'])));
        }
        if(isset($row['is_system'])){
            $this->where(array('is_system'=>$row['is_system']));
        }
        if($row['ids']){
            $this->where(array('id'=>array('in',$row['ids'])));
        }
